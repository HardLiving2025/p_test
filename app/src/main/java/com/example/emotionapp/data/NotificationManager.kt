                ...
                .enqueue(
                        object : Callback {
                            override fun onFailure(call: Call, e: IOException) {
                                Log.e("NotificationManager", "Failed to fetch notifications", e)
                                ...
                                response.use {
                                    ...
                                    if (jsonString != null) {
                                        ...
                                        try {
                                            ...
                                            val jsonArray =
                                                    jsonObject.getJSONArray("recent_notifications")
                                            val list = mutableListOf<NotificationFromServer>()

                                            for (i in 0 until jsonArray.length()) {
                                                val item = jsonArray.getJSONObject(i)
                                                list.add(
                                                        NotificationFromServer(
                                                                noti_id = item.getInt("noti_id"),
                                                                message_type =
                                                                        item.getString(
                                                                                "message_type"
                                                                        ),
                                                                ...
                                                                sent_at = item.getString("sent_at")
                                                        )
                                                )
                                            }
                                            onResult(list)
                                        } catch (e: Exception) {
                                            Log.e("NotificationManager", "Parsing error", e)
